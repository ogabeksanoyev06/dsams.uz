  ...
  <UIModal disableOuterClose :model-value title="Avtorizatsiya" @update:model-value="emit('update:modelValue', $event)">
    <div class="pt-4 p-5">
      <Transition name="fade" mode="out-in">
        <FormGroup v-if="step === 1" label="Elektron pochtangizni kiriting" for-id="email">
      ...
      <UIButton class="w-full mt-5 !border-none" :text="step === 1 ? 'Davom etish' : 'Kirish'" :disabled="step === 2 && !form.otp" :loading @click="submit" />
    </div>
  </UIModal>
</template>
