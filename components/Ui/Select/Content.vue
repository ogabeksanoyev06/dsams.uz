<template>
  <UiSelectPortal :to="to">
    <SelectContent v-bind="{ ...forwarded, ...$attrs }" :class="styles({ position, class: props.class })">
...
<script lang="ts" setup>
  ...
  const styles = tv({
    base: "relative z-[999] min-w-[var(--radix-select-trigger-width)] w-full max-h-[250px] overflow-hidden rounded-md border bg-popover text-accent-foreground shadow-md data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2",
    variants: {
      position: {
