<template>
  <div class="code-input" :class="error ? 'error' : ''">
    <label v-if="label" class="leading-130 mb-2 block text-sm font-medium" :for="`verification-input-${randomNumber}`">
      {{ label }}
    </label>
      ...
      <template v-for="(v, index) in values" :key="index">
        <div class="relative overflow-hidden !rounded-none">
          <input
            ...
            "
            class="transition-300 leading-140 h-14 w-full overflow-hidden rounded-xl border border-transparent bg-secondary text-center text-xl font-semibold outline-none focus-within:border-primary focus-within:bg-white"
