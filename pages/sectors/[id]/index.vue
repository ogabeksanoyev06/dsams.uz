<template>
  <div class="container py-4 pb-8">
    ...
    <div class="column-1 mb-6 gap-3 space-y-4 md:columns-2 lg:columns-3 xl:gap-4">
      <div class="break-inside-avoid" v-for="(item, key) in sector?.data?.sections" :key>
        <div class="cursor-pointer rounded-xl bg-background p-3 font-medium sm:p-4 min-h-[70px] flex items-center justify-center text-center">
          {{ item.name }}
        </div>
      </div>
    </div>
